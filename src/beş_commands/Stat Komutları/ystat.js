...
module.exports = {
    name: "ystat",
    usage: "ystat [Official Development / ID]",
    category: "stat",
    ...
    execute: async (client, message, args, beş_embed) => {
		...
		if (beş_config.taskSystem && member.hasRole(beş_config.staffs, false) && client.ranks.length > 0) {
			beş_embed.addFields([{name:`**${client.emoji("emote_value").id !== null ? client.emoji("emote_value").id:"🔹"} Puan Durumu:**`, value:`\n${client.progressBar(pointData ? Math.floor(pointData.point) : 0, maxValue.point, 8)} \`${pointData ? Math.floor(pointData.point) : 0} / ${maxValue.point}\``}]);
			beş_embed.addFields([{name:
				`**${client.emoji("emote_value").id !== null ? client.emoji("emote_value").id:"🔹"} Yetki Durumu:**`,value:
