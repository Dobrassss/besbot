    ...
    execute: async (client, message, args, beş_embed) => {
        ...
        let buttons = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                ...
                .setEmoji(`${client.emoji("emote_chat").id !== null ? client.emoji("emote_chat").id : "💭"}`),
            new ButtonBuilder()
                .setLabel("Ses Görevi")
                .setCustomId("task-voice")
                .setStyle(ButtonStyle.Primary)
                .setEmoji(`${client.emoji("emote_voice").id !== null ? client.emoji("emote_voice").id : "🔉"}`),
            new ButtonBuilder()
                .setLabel("Davet Görevi")
                .setCustomId("task-invite")
                .setStyle(ButtonStyle.Primary)
                .setEmoji(`${client.emoji("emote_invite").id !== null ? client.emoji("emote_invite").id : "📩"}`),
            new ButtonBuilder()
                .setLabel("Taglı Görevi")
                .setCustomId("task-tag")
                .setStyle(ButtonStyle.Primary)
                .setEmoji(`${client.emoji("emote_others").id !== null ? client.emoji("emote_others").id : "🔹"}`),
            new ButtonBuilder()
                .setLabel("Kayıt Görevi")
                .setCustomId("task-reg")
                .setStyle(ButtonStyle.Primary)
                .setEmoji(`${client.emoji("emote_others").id !== null ? client.emoji("emote_others").id : "🖋️"}`),
        )
       ...
       if(tasks.filter((x) => x.active).length >= 3)return message.reply({embeds:[beş_embed.setDescription(`> **En Fazla 3 Aktif Görevin Bulunabilir, Yeni Görev Almak İçin Eski Görevlerini Bitirmelisin!**`)]}).sil(5);
        ...
        let mesaj = await message.reply({ components: [buttons], embeds: [beş_embed.setDescription(`> **${message.member} Aşağıdaki Görev Tiplerinden Birini Seçiniz.**\n> **Seçtiğiniz Tipe Göre Rastgele Adetli Görev Verilecek!**\n\n> **${client.emoji("emote_chat").id !== null ? client.emoji("emote_chat").id : "💭"} Mesaj Görevi;** *Sunucudaki ${db.has("five-channel-chat") ? message.guild.channels.cache.get(db.get("five-channel-chat")) : "Genel Sohbet"} Kanalında Mesaj Yazma Görevi.*\n\n> **${client.emoji("emote_voice").id !== null ? client.emoji("emote_voice").id : "🔉"} Ses Görevi;** *Sunucu Sesli Kanallarında Bulunma / Vakit Geçirme Görevi.*\n\n> **${client.emoji("emote_invite").id !== null ? client.emoji("emote_invite").id : "📩"} Davet Görevi;** *Sunucumuza Yeni Üyeler Davet(Invite) Etme Görevi.*\n\n> **${client.emoji("emote_others").id !== null ? client.emoji("emote_others").id : "🔹"} Taglı Görevi;** *Üyelere / Kullanıcılara Tag Aldırma Görevi.*\n\n> **${client.emoji("emote_others").id !== null ? client.emoji("emote_others").id : "🖋️"} Kayıt Görevi;** *Sunucumuza Yeni Gelen Üyeleri Karşılama / Kayıt Etme Görevi.*`).setThumbnail(message.author.displayAvatarURL({ dynamic: true }))] })
